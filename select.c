...
*/
/*
** 删除select结构中的内容，但不释放select结构
*/
static void clearSelect(sqlite3 *db, Select *p){
  sqlite3ExprListDelete(db, p->pEList);   /*删除整个表达式列表*/  //删除表达式
  sqlite3SrcListDelete(db, p->pSrc);   /*删除整个SrcList,包括所有的子结构*/ //删除from
  sqlite3ExprDelete(db, p->pWhere);     /*递归删除一个表达式树*/  //删除where
  sqlite3ExprListDelete(db, p->pGroupBy);   /*删除整个表达式列表*/  //删除group by
  sqlite3ExprDelete(db, p->pHaving);   /*递归删除一个表达式树*/  //删除having
  sqlite3ExprListDelete(db, p->pOrderBy);   /*删除整个表达式列表*/  //删除order by
  sqlite3SelectDelete(db, p->pPrior);   /*删除给定的选择结构和所有的子结构*/  //删除与前一个select的链接
  sqlite3ExprDelete(db, p->pLimit);    /*递归删除一个表达式树*/   //删除limit
  sqlite3ExprDelete(db, p->pOffset);     /*递归删除一个表达式树*/   //删除offset
}
...
void sqlite3SelectDestInit(SelectDest *pDest, int eDest, int iParm){/*函数sqlite3SelectDestInit的参数列表为
                                                                      ...
                                                                      整型指针iParm
                                                                      */
  pDest->eDest = (u8)eDest; /*把整型eDest 强制类型转化为unsigned char型，然后赋值给pDest->eDest*/
...
Select *sqlite3SelectNew(
  ...
  Select *pNew;/*定义结构体指针pNew*/  //定义新的Select结构指针
  Select standin;/*定义结构体类型变量standin*/
  sqlite3 *db = pParse->db;/*结构体Parse的成员db赋值给结构体sqlite3指针db*/
  pNew = sqlite3DbMallocZero(db, sizeof(*pNew) );  /* 分配和零内存，如果分配失败，使mallocFaied标志在连接指针中。 */  //分配内存给pNew
  assert( db->mallocFailed || !pOffset || pLimit ); /* 判断分配是否失败,或pOffset值为空,或pLimit值不为空*/ //若db分配成功，则不能使pOffset不为空，而pLimit为空。这条语句其实没多大用
  if( pNew==0 ){/*如果结构体指针变量pNew指向的地址为0*/ //解决分配失败
    assert( db->mallocFailed ); //表明数据库分配失败
    pNew = &standin;/*把standin的存储地址赋给pNew*/
    memset(pNew, 0, sizeof(*pNew));  //重新分配内存给pNew，并将其中的数据初始化为0
  }
  if( pEList==0 ){/*如果结构体指针pEList指向的地址为0*/
    pEList = sqlite3ExprListAppend(pParse, 0, sqlite3Expr(db,TK_ALL,0)); /*新添加的元素在表达式列表的末尾。新添加元素 //如果pEList为空，则将其赋值为查询数据库的全部内容
  ...
  if( pSrc==0 ) pSrc = sqlite3DbMallocZero(db, sizeof(*pSrc));  //如果pSrc为空，则分配新的内存给pSrc
  pNew->pSrc = pSrc;
  pNew->pWhere = pWhere;
  ...
  if( db->mallocFailed ) {/*如果内存分配失败*/ //验证分配是否成功，若失败则清空所有参数中的内容
...
** 提供1到3个连接关键字，决定连接类型。返回下面的整型常量组合来表示
**     JT_INNER
**     JT_CROSS
**     JT_OUTER
**     JT_NATURAL
...
**返回一个整数常数,表示该类型的下列值: 
** 如果是一个非法或不支持的连接类型，也返回一个连接类型，但会放置一个错误信息给
** Parse结构。
**JT_INNER 
**JT_CROSS 
**JT_OUTER 
...
int sqlite3JoinType(Parse *pParse, Token *pA, Token *pB, Token *pC){
  ...
  static const struct {/*定义只读的且只能在当前模块可见结构体*/
    u8 i;        /* Beginning of keyword text in zKeyText[]   在KeyText[] 中开始关键字的文本*/ //关键字在zKeyText中的起始位
    u8 nChar;    /* Length of the keyword in characters  在字符中关键字的长度*/ //关键字字符长度
    u8 code;     /* Join type mask */ //连接类型
  } aKeyword[] = {
  ...
  for(i=0; i<3 && apAll[i]; i++){
    ...
    for(j=0; j<ArraySize(aKeyword); j++){
      if( p->n==aKeyword[j].nChar 
        ...
        jointype |= aKeyword[j].code;//在aKeyword中寻找对应的token值，并求出连接类型。
        break;
      }
    }
    testcase( j==0 || j==1 || j==2 || j==3 || j==4 || j==5 || j==6 );//检测j是否越界
    if( j>=ArraySize(aKeyword) ){
      jointype |= JT_ERROR;//如果j越界，则报错
      break;
    }
  }
  if(
...
** 从左到右搜索pSrc中的前N个表，查找含有zCol列的表
** When found, set *piTab and *piCol to the table index and column index
** of the matching column and return TRUE.
**当找到以后，设置* piTab和* piCol表索引和匹配列的列索引，并返回TRUE 。
** 找到后，设置*piTab和*piCol指向表和列的索引，并返回真。否则返回假
** If not found, return FALSE.
**如果没有找到，返回FALSE。
*/
static int tableAndColumnIndex(
  SrcList *pSrc,       /* Array of tables to search 搜索一系列的表*/ //搜索的表数组
  int N,               /* Number of tables in pSrc->a[] to search 在pSrc->a[] 中搜索表的数目*/ //表数组中有多少个表需搜索
  const char *zCol,    /* Name of the column we are looking for 寻找列的名字*/ //要搜索的列名
  int *piTab,          /* Write index of pSrc->a[] here 写入索引*/ //记录表的索引值
  int *piCol           /* Write index of pSrc->a[*piTab].pTab->aCol[] here 写入索引*/ //记录列的索引值
){
  int i;               /* For looping over tables in pSrc 对pSrc遍历表*/
  int iCol;            /* Index of column matching zCol   匹配zCol索引的列*/ //列的索引值

  assert( (piTab==0)==(piCol==0) );  /* Both or neither are NULL  都是或都不是都为空*/ //同时为空或同时有值
  for(i=0; i<N; i++){
...
** 该函数为select中的where语句添加含有join语法的术语。比如下面这样的语句
** 格式:
**    (tab1.col1 = tab2.col2)
**
...
** tab1是SrcList中第iSrc个表，tab2是第iSrc+1个表。col1在tab1中，用iColLeft
** 表示，col2在tab2中，用iColRight表示。
...
static void addWhereTerm(
  ...
  int iLeft,                      /* Index of first table to join in pSrc  第一个表的索引加入pSrc */ //pSrc中连接的第一个表的索引
  int iColLeft,                   /* Index of column in first table  第一个表的列的索引*/
  int iRight,                     /* Index of second table in pSrc  第二个表的索引在 pSrc中*/ 
  int iColRight,                  /* Index of column in second table  列的索引在第二个表中*/ //pSrc中连接的第二个表的索引
  int isOuterJoin,                /* True if this is an OUTER join  如果是外部连接返回邋true*/ //为真表示是外连接
  ...
  assert( iLeft<iRight );//iLeft必须小于iRight
  assert( pSrc->nSrc>iRight );//iRight不能超过pSrc->nSrc
  assert( pSrc->a[iLeft].pTab ); //pSrc->a[iLeft].pTab不能为空
  assert( pSrc->a[iRight].pTab );//pSrc->a[iRight].pTab不能为空


  pE1 = sqlite3CreateColumnExpr(db, pSrc, iLeft, iColLeft);/*分配并返回一个表达式的指针，来加载在SrcList pSrc中的 //以第一个表的iColLeft列创建表达式 
                                                             数据源的iCol列。把这个指针赋给pE1。
                                                            */
  pE2 = sqlite3CreateColumnExpr(db, pSrc, iRight, iColRight);//以第二个表的iColRight列创建表达式

  pEq = sqlite3PExpr(pParse, TK_EQ, pE1, pE2, 0);  //将pE1和pE2组合为表达式
  if( pEq && isOuterJoin ){/*如果定义结构体指针pEq指向的地址非空且isOuterJoin为真*/ //检测pEq和连接类型
    ExprSetProperty(pEq, EP_FromJoin); //设置pEq的flags为EP_FromJoin
    assert( !ExprHasAnyProperty(pEq, EP_TokenOnly|EP_Reduced) );
    ExprSetIrreducible(pEq); //设置pEq的flags2为EP2_Irreducible
    pEq->iRightJoinTable = (i16)pE2->iTable;//将第二个表中iColRight的游标号给pEq->IRightJoinTable
  }
  *ppWhere = sqlite3ExprAnd(db, *ppWhere, pEq);//将pEq添加到*ppWhere中
}
...
** 设置EP_FromJoin属性给传入的表达式，并设置表达式中的iRightJoinTable为
** 传入的iTable。
...
** EP_FromJoin用于通知左外连接的逻辑进程，连接的详细限定语句在ON或USING
** 中，而不是WHERE语句。这些语句被移到WHERE中，但我们要记住它们其实是在
** ON或USING中。
...
** for cases like this:
** 表达式的iRightJoinTable通知WHERE语句进程，该表达式依赖iRightJoinTable
** 指定的表，即使该表没有出现在表达式中。比如下面的语句：
...
** 该语句将在处理完t2的连接后，才处理t1.x=5。即使a为NULL，t2的行也会
** 被插入到t1.!=5中。如果不推迟处理t1.x=5，则不会出现前面结果。
**SELECT * FROM t1 LEFT JOIN t2 ON t1.a=t2.b AND t1.x=5
...
**自然连接也创建额外的where子句条件。
** 此例程处理select语句中的连接信息。on、using和natural会转换为特殊的where
** 语句
...
** from语句保存在Select.pSrc结构中。最左边的表是Select.pSrc的第一个进入点。
** 最右边的表是最后一个入口。连接操作保留左边的入口，所以入口0表示连接入口0和1。
** 任何on或using语句也依附于左入口。
...
static int sqliteProcessJoin(Parse *pParse, Select *p){
  SrcList *pSrc;                  /* All tables in the FROM clause   rom子句中的所有表*/ //含有from语句中的全部表
  int i, j;                       /* Loop counters  循环计数器*/
  struct SrcList_item *pLeft;     /* Left table being joined   左表被加入*/ //左边的表
  struct SrcList_item *pRight;    /* Right table being joined   右表被加入*/ //右边的表
  ...
  for(i=0; i<pSrc->nSrc-1; i++, pRight++, pLeft++){
    ...
    ** 当有natural关键字时，为两个表中所有共同列添加where语句
	*/
    if( pRight->jointype & JT_NATURAL ){
      ...
      for(j=0; j<pRightTab->nCol; j++){
        char *zName;   /* Name of column in the right table 右表中列的名字*/ //右表的列名
        int iLeft;     /* Matching left table 匹配左表*/ //匹配的左表索引值
        int iLeftCol;  /* Matching column in the left table 在左表中匹配列*/ //匹配的左表列索引值
    ...
    **一个错误。
	** 为每一个using语句中的列名创建特殊的术语到where语句中。比如：如果
	** 两个表A和B进行连接，调用using时使用列名X,Y和Z，将添加：
	** A.X=B.X AND A.Y=B.Y AND A.Z=B.Z到where中。如果有任意一个列名不在两个
	** 表中，则报错。
    */
    if( pRight->pUsing ){/*如果结构体指针pRight引用的成员变量pUsing非空*/
      IdList *pList = pRight->pUsing;/*把pRight->pUsing赋给结构体指针pList*/
      for(j=0; j<pList->nId; j++){
        char *zName;     /* Name of the term in the USING clause   using子句的名称术语*/ //using中的列名
...
static void pushOntoSorter(
  ...
  int regData            /* Register holding data to be sorted  注册数据进行排序*/ //保存排序数据
){
  Vdbe *v = pParse->pVdbe;
  int nExpr = pOrderBy->nExpr;
  int regBase = sqlite3GetTempRange(pParse, nExpr+2); /*分配或释放一块连续的寄存器，返回一个整数值，//获取临时范围
                                                        把该值赋给regBase。
                                                       */
  int regRecord = sqlite3GetTempReg(pParse); /*分配一个新的寄存器用于控制中间结果。*/
  int op;
  sqlite3ExprCacheClear(pParse); /*清除所有列的缓存条目*/ //清空cache中的Parse
...
static void codeOffset(
  Vdbe *v,          /* Generate code into this VM  在VM中生成代码*/ //生成代码给VM
  Select *p,        /* The SELECT statement being coded  select语句被编码*/ //被编码的select语句
  int iContinue     /* Jump here to skip the current record  跳到这里跳过当前记录*/ //跳转目标用以跳过当前记录
){
...
** 添加代码以确保N个寄存器在iMem中的起始位置不同。iTab是保存了N个值排列的顺序
** 索引。如果当前N个值是新的。iTab将生成一个新入口。
** A jump to addrRepeat is made and the N+1 values are popped from the
** stack if the top N elements are not distinct.
**如果前N个元素不明显，跳转到addrRepeat，N+1个值从栈中弹出。
** 如果栈顶的N个值相同，则将N+1个值出栈并跳转到addrRepeat
*/
...
** 当select是子查询时，返回结果超过1个列时，生成错误信息。
*/
...
** 如果srcTab和nColumn都为零，则对pElist表达式求值得到该行的数据。如果
** nColumn>0，则从srcTab和pElist
*/
static void selectInnerLoop(
  Parse *pParse,          /* The parser context 语义分析*/
  Select *p,              /* The complete select statement being coded 完整的select语句被编码*/
  ExprList *pEList,       /* List of values being extracted  输出结果列的语法树*/ //被提取的值的列表
  int srcTab,             /* Pull data from this table 从这个表中提取数据*/ //从该表提取数据
  int nColumn,            /* Number of columns in the source table  源表中列的数目*/ //表中的列数
  ExprList *pOrderBy,     /* If not NULL, sort results using this key 如果不是NULL，使用这个key对结果进行排序*/ //不为空，用其进行排序
  int distinct,           /* If >=0, make sure results are distinct 如果>=0，确保结果是不同的*/ //如果不小于0，确保结果唯一
  SelectDest *pDest,      /* How to dispose of the results 怎样处理结果*/ //如何处理结果
  int iContinue,          /* Jump here to continue with next row 跳到这里继续下一行*/ //跳到此处继续处理下一行
  int iBreak              /* Jump here to break out of the inner loop 跳到这里中断内部循环*/ //跳到此处结束内循环
){
  Vdbe *v = pParse->pVdbe;
  int i;
  int hasDistinct;        /* True if the DISTINCT keyword is present 如果distinct关键字存在返回true*/ //如果有distinct关键字，值为真
  int regResult;              /* Start of memory holding result set 开始的内存持有结果集*/ //保存结果集合的内存起始位置
  int eDest = pDest->eDest;   /* How to dispose of results 怎样处理结果*/
  int iParm = pDest->iSDParm; /* First argument to disposal method 第一个参数的处理方法*/ //处理方法的第一个参数
  int nResultCol;             /* Number of result columns 结果列的数目*/ //结果的列数
  ...
  /* Pull the requested columns. 提取请求的列
  **取出请求列
  */
  ...
  ** 如果select中有distinct关键字，且改行已经出现过，则抛弃该行。
  */
  if( hasDistinct ){
    assert( pEList!=0 );
  ...
  switch( eDest ){/*定义switch函数，根据参数eDest判断怎样处理结果*/
	...
	** 将查询结果构建为一个记录，但不是保存记录，而是作为键来删除临
	** 时表iParm中的数据。
    */
	...
	** 如果我们创建了一个集合给"expr IN (SELECT ...)"结构，那么栈上应该有
	** 一个单项。将其与虚假数据写入到集合表中。
    */
    case SRT_Set: {/*如果eDest为SRT_Set，则结果作为关键字存入索引*/
      ...
      if( pOrderBy ){
        ...
        **因为项目的集合中的顺序并不重要。
		** 乍一看，你可能认为在这种情况下我们可以优化order by，因为set中顺序不重要。
		** 但这有可能是limit语句，这时就需要考虑顺序。
        */
	...
	** 如果一个标量选择是表达式的一部分，那么将结果存储在合适的内存单元
	** 并跳出循环。
    */
	...
	** 传送数据给回调函数或子例程。传送给子例程时，子例程将负责把数据从堆栈
	** 弹出。
    */
    ...
    **我们不关心实际的选择结果。
	** 丢弃该结果。这需要select中的trigger来调用。其目的是调用用户自定义的
	** 函数。我们不用关心其实际结果。
    */
    default: {/*默认条件下*/
  ...
  **在这种情况下分选机已经限制了我们的输出。
  ** 如果含有limit则跳出循环。此外，如果有排序语句，则输出结果已经被限制。
  */
...
** 提供一个表达式列表，生成一个KeyInfo结构为记录中的表达式排序。
...
**现DISTINCT测试。
** 如果列表中是order by或group by语句，则生成的KeyInfo结构适用于初始化一个
** 虚拟索引来执行列表中的语句。如果列表中是select的结果集，那么KeyInfo结构
** 适用于初始化虚拟索引来执行distinct语句。
...
** KeyInfo分配到的内存空间，由调用函数负责释放。通常将KeyInfo传送到P4中使用
** 了P4_KEYINFO_HANDOFF操作码的部分来处理其释放。
*/
...
** 除非正在一个“EXPLAIN QUERY PLAN”命令，此时函数执行空操作。否则，将添加
** 一行如下格式的标题到EQP的输出结果中
...
**zUsage参数决定。
** 其中xxx是distinct，order by和group by其中的一个。准确来说取决于参数zUsage。
*/
...
** 下面的宏将b赋值给a。还有一个空操作版本，是当SQLITE_OMIT_EXPLAIN被定义了时。
** 此宏允许sqlite3Select()给结构成员变量赋值,只有当SQLITE_OMIT_EXPLAIN
** 没有定义,没有跳过的ifndef指令中代码时才执行。
*/
...
**除非一个"EXPLAIN QUERY PLAN"命令正在处理，这个功能就是一个空操作。
**否则，它增加一个单独的输出行到EQP结果，标题的形式为:
** 整型iSub1和iSub2作为相应的传递函数参数,操作符使用与文本表达同名做参数。
** 参数“op”必须为TK_UNION,TK_EXCEPT,TK_INTERSECT或TK_ALL之一。
** 如果参数bUseTmp为假时,使用第一个格式。如果为真，使用第二种形式。
**"COMPOSITE SUBQUERIES iSub1 and iSub2 (op)"
**"COMPOSITE SUBQUERIES iSub1 and iSub2 USING TEMP B-TREE (op)"
...
** 如果内循环生成一个使用非空的pOrderBy的参数，结果将放入一个排序器中。循环
** 终止后，我们需要调用排序函数对结果排序并输出。以下例程生成所需的代码。
*/
